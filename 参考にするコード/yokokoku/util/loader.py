...
class Loader(object):
    def __init__(self, dir_original, dir_segmented, init_size=(128, 128), one_hot=True):
        self._data = Loader.import_data(
            dir_original, dir_segmented, init_size, one_hot)
    ...
    def generate_paths(dir_original, dir_segmented):
        ...
        filenames = list(map(lambda path: path.split(os.sep)
                             [-1].split(".")[0], paths_segmented))
        paths_original = list(
    ...
    def extract_images(paths_original, paths_segmented, init_size, one_hot):
        ...
        images_segmented = np.where(images_segmented == 255, len(
            DataSet.CATEGORY)-1, images_segmented)
...
class DataSet(object):
    ...
    def __init__(self, images_original, images_segmented, image_palette, augmenter=None):
        assert len(images_original) == len(
            images_segmented), "images and labels must have same length."
    ...
    def __add__(self, other):
        images_original = np.concatenate(
            [self.images_original, other.images_original])
    ...
    def shuffle(self):
        ...
        self._images_original, self._images_segmented = self._images_original[
            idx], self._images_segmented[idx]
...
if __name__ == "__main__":
    dataset_loader = Loader(dir_original="../data_set/VOC2012/JPEGImages",
